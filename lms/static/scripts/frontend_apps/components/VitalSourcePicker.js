...
export default function VitalSourcePicker({
  ...
  const [step, setStep] = useState('select-book');
  ...
  useEffect(() => {
    if (step === 'select-book' && !bookList) {
      ...
      apiCall({ authToken, path: `/api/vitalsource/books/${book.id}/toc` })
...
VitalSourcePicker.propTypes = {
  authToken: propTypes.string,

  onCancel: propTypes.func,

  /** Callback invoked when user selects a book. */
